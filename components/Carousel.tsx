...
const CarouselItem = ({ imageUrl, id, title, undertitle }: ICarouselItem) => {
    return (
        <Box bgImage={imageUrl} backgroundSize={"cover"} backgroundPosition={"center"} h="155" borderRadius={"12px"}>
            <VStack gap="0px" justifyContent={"flex-end"} h="100%" alignItems={"flex-start"} p="4">
                <Text noOfLines={1} fontWeight={"semibold"} fontSize={"16px"} color="white.900">
                    {title}
                </Text>
                <Text noOfLines={1} fontWeight={"semibold"} fontSize={"10px"} color="white.900">
                    {undertitle}
                </Text>
            </VStack>
        </Box>
